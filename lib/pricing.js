...
function groupByEmail(rows) {
  ...
  for (const row of rows) {
    ...
    grouped[email].items.push({
      ...
      embroideryPosition: row["Embroidery Position"] || "",
...
/**
 * Calculates Stripe processing fee such that after Stripe takes 2.9% + $0.30
 * on the full invoice (including this fee), the net equals the original subtotal.
 * @param {number} subtotal - Order subtotal (items + tax + shipping)
 * @returns {number} Fee amount to charge the customer
 */
function calculateStripeFee(subtotal) {
  return (subtotal + 0.30) / (1 - 0.029) - subtotal;
}
...
function formatItemDescription(item) {
  ...
  if (item.embroideredName) {
    desc += `\nEmbroidered: "${item.embroideredName}" (${item.threadColor} thread, ${item.embroideryPosition})`;
  }
  return desc;
}
