 ...
 *
 * Usage:
 *   node invoice.js orders.csv              # Create draft invoices
 *   node invoice.js orders.csv --send       # Create and send invoices
 *   node invoice.js orders.csv --dry-run    # Preview without creating
 *
 * Requires:
 *   - .env file with STRIPE_SECRET_KEY
...
if (!csvFile) {
  console.error("Usage: node invoice.js <orders.csv> [--dry-run] [--send]");
  console.error("");
  console.error("Options:");
  console.error("  --dry-run  Preview invoices without creating them in Stripe");
  console.error("  --send     Automatically send invoices (default: create as drafts)");
  process.exit(1);
}
